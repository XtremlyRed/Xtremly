...
namespace Xtremly.Core
{
    ...
    internal sealed class ProtocolPacket : IDisposable
    {
        ...
        private static int CommandCounter = int.MinValue;
        ...
        public byte[] ToBuffer()
        {
            ...
            byte[] buffer = new byte[TotalHeaderLength + DataLength];
        ...
        public static ProtocolPacket FromBuffer(byte[] buffer, int offset, int bufferLength, Func<byte[], int, int, byte[]> decompress)
        {
            ProtocolPacket protocol = new()
            {
                PacketLength = BitConverter.ToInt32(buffer, PacketLengthIndex + offset),
                Counter = BitConverter.ToInt32(buffer, PacketCounterIndex + offset),
                IsCompress = buffer[PacketIsCompressIndex + offset] == 1,
                ReportArrived = buffer[PacketReportArrivedIndex + offset] == 1
            };
            ...
            byte[] buffer2 = new byte[protocol.DataLength];
        ...
        internal static ICollection<ProtocolPacket> FromBuffers(byte[] buffer, int offset, int bufferLength, Func<byte[], int, int, byte[]> decompress)
        {
            List<ProtocolPacket> list = new();
            do
            {
                ...
                {
                    ProtocolPacket pp = FromBuffer(buffer, offset, buffer.Length, decompress);
                    list.Add(pp);
        ...
        {
            using MemoryStream compressStream = new(bytes, offset, length);
